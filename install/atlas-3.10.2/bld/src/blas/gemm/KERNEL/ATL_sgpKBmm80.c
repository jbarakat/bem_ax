...
#endif
#endif
static void ATL_sJIK0x0x80TN1x1x6_a1_bX
 ...
 */
{
   ...
   #undef Kb
#endif
void ATL_sJIK0x0x80TN80x80x0_a1_bX
   (const int M, const int N, const int K, const float alpha, const float * ATL_RESTRICT A, const int lda, const float * ATL_RESTRICT B, const int ldb, const float beta, float * ATL_RESTRICT C, const int ldc)
/*
 * matmul with TA=T, TB=N, MB=0, NB=0, KB=80, 
 ...
 */
{
   ...
   if (k=M-Mb)
      ATL_sJIK0x0x80TN1x1x6_a1_bX(k, N, 80, alpha, ca + (((Mb) << 6)+((Mb) << 4)), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
