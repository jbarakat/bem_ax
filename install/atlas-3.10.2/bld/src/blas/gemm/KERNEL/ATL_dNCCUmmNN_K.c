...
static void ATL_dJIK0x0x24NN1x1x2_aX_bX
 ...
 */
{
   ...
   const double BetaAlpha = beta / alpha;
   const int incAk = (lda);
   const int incAm = 12 - (((lda) << 4)+((lda) << 3)), incAn = -(Mb);
   #define incBk 2
   const int incBm = -24, incBn = (ldb);
   #define incCm 12
   const int incCn = (ldc) - (Mb);
   ...
   if (k=M-Mb)
      ATL_dJIK0x0x24NN1x1x2_aX_bX(k, N, 24, alpha, ca + (Mb), lda, cb, ldb, beta, cc + (Mb), ldc);
}
#ifdef incAm
   #undef incAm
#endif
